...
public class FileBrowserEngine {
    ...
    public AdapterData loadDir(File directory) {
        ...
        if (files != null) {
            ...
            if(parent != null) {
                pathsList.add(parent.getAbsolutePath());
            } else {
                parent = directory;
                pathsList.add(parent.getAbsolutePath());
            }

            File[] listOfFilesParent = parent.listFiles();
            if (parent.listFiles().length == 1) {
                sizesList.add("" + listOfFilesParent.length + " item");
            } else {
                sizesList.add("" + listOfFilesParent.length + " items");
            }
                ...
                if ((!file.isHidden() || mFileBrowserView.shouldShowHiddenFiles()) && file.canRead()) {
                    ...
                    if (file.isDirectory() && mFileBrowserView.shouldShowFolders()) {
                        ...
                        try {
                            ...
                            if (mFileBrowserView.getFileExtensionFilter() != null && mFileBrowserView.getFileExtensionFilter()
                                    .getFilterList().contains(ext))
                                continue;
                        ...
                        } else if (getFileExtension(fileName).equalsIgnoreCase("txt")) {
